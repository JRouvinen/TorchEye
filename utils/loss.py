...
# This implementation is based on https://github.com/WongKinYiu/PyTorch_YOLOv4/blob/master/utils/loss.py
...
def smooth_BCE(eps=0.1):  # YoloV4 implementation - "0.4.0 - RC3"
...
def compute_loss(predictions, targets, model): # YoloV4 implementation - "0.4.0 - RC3"
    ...
    # Define criteria
    BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([float(h['cls_pw'])])).to(device)
    BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([float(h['obj_pw'])])).to(device)

    # Class label smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3
    cp, cn = smooth_BCE(eps=0.0)

    # Focal loss
    g = float(h['fl_gamma'])  # focal loss gamma
    if g > 0:
    ...
    lbox *= float(h['box']) * s
    lobj *= float(h['obj']) * s * (1.4 if no >= 4 else 1.)
    lcls *= float(h['cls']) * s
...
def training_fitness(x,w):
    # Model training fitness as a weighted combination of training metrics
